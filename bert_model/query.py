...
with open('./bert_model/mh_emb.pkl', 'rb') as handle:
...
def call_lsi(search):
    '''Calls the LSI model & returns the row index of the top result in df.csv. Inference time takes 50% LONGER!!!'''
    with open('./bert_model/dic.pkl', 'rb') as handle:
        dic = pickle.load(handle)
    with open('./bert_model/corpus.pkl', 'rb') as handle:
